...
// 简化版：只尝试连接数据库，不执行同步或迁移，即使连接失败也继续启动服务
const startServer = async () => {
  try {
    // 尝试连接数据库（仅测试连接，不执行任何初始化操作）
    await sequelize.authenticate();
    console.log('Database connected successfully');
  } catch (err) {
    // 数据库连接失败，但继续启动服务
    console.warn('Database connection failed, but server will continue to start:', err.message);
  }

  // 直接启动服务器，不执行 sequelize.sync 或数据库迁移
